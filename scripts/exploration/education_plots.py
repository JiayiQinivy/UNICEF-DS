...
EDUCATION_CSV    = os.path.join(SCRIPT_DIR, "education_clean.csv")
...
def plot_regional_boxplot(merged):
    ...
    out = os.path.join(OUTPUT_DIR, "education_regional_boxplot.png")
    fig.savefig(out, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    for path, label in [(EDUCATION_CSV,    "education_clean.csv"),
    ...
    print(f"{'='*60}")
    print("\nOutput files:")
    for f in ["education_correlation_heatmap.png",
              "education_scatter_plots.png",
              "education_regional_boxplot.png"]:
